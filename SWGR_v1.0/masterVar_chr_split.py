...
def chr_split_mastervar(f_path, target_path):
    ...
    for mastervar in chunk:  #iterate through mastervar file
        for current_chrom,chr_df in mastervar.groupby(['chromosome']):  #split dataframe by chromosome for writing
            ...
            if len(os.popen('find '+ write_target_file_path + '').readlines()) == 0:  #do not write header
                os.system('bzcat '+ f_path + '| head -' + str(skip_rows) + " > " +write_target_file_path)
                chr_df.to_csv(write_target_file_path, sep="\t", index=False, mode='a')
            else:  #Write header if target file empty
                chr_df.to_csv(write_target_file_path, sep="\t", index=False, mode='a', header=False)
